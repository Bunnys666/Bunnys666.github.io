...
install pdf to text from python library with following command:
...
run linpeas.sh and found writable logrotate

![](/assets/img/book/17.png)

running pspy for monitoring background prosess
...
basiclly logrotate is the process that renames a current log file (e.g auth.log becomes auth.log.1) and set up a new log files.
...
prepare for our payload in (ssh reader)

create file and given name payload with contain:
...
compiling the exploit on host and send back into reader using wget or curl.

execute exploit with following command:

```bash
./exploit -p ./payload /home/reader/backups/access.log
```

open another session via ssh, and execute this command below will lead you into root
...
now execute /bin/bash -p you will be root
